...
Object.defineProperty(globalThis, "localStorage", {
  value: {
    ...
    setItem: (key: string, value: string) => testLocalStorage.set(key, String(value)),
...
const { terminalGroups, logMeta, isCompactionTurnEndedLog } = await import("../public/js/terminal-groups.js");
...
const { filePreviewLanguage } = await import("../public/js/file-preview.js");
...
const {
  ...
  streamingTerminalSnapshot,
  streamingTerminalGroup,
} = await import("../public/js/terminal-render.js");
const legacyFlowName = new RegExp(`${"water"}${"flow"}`, "i");

test("streaming tail updates match full grouping and fall back on structural changes", () => {
  const flow = { id: "streaming-tail-test", agentStatus: "running", agentModel: "gpt-6-astra" };
  const logs = [
    { id: 1, flowId: flow.id, source: "user", message: "hello", createdAt: "2026-09-15T12:00:00Z" },
    { id: 2, flowId: flow.id, source: "agent:message", message: "Hello", createdAt: "2026-09-15T12:00:01Z" },
  ];
  const snapshot = streamingTerminalSnapshot(logs, terminalGroups(logs, flow), flow);
  expect(snapshot).not.toBeNull();
  expect(streamingTerminalGroup(snapshot, logs, flow)).toBeNull();
  logs[1].message += " world";
  expect(streamingTerminalGroup(snapshot, logs, flow)).toEqual(terminalGroups(logs, flow).at(-1));
  expect(streamingTerminalGroup(snapshot, [...logs], flow)).toBeNull();
  expect(streamingTerminalGroup(snapshot, logs, { ...flow, agentStatus: "idle" })).toBeNull();
  state.rawAgentMarkdownByFlow.set(flow.id, !snapshot.raw);
  expect(streamingTerminalGroup(snapshot, logs, flow)).toBeNull();
  state.rawAgentMarkdownByFlow.delete(flow.id);
  logs.push({ id: 3, flowId: flow.id, source: "agent:message-boundary", message: "", createdAt: "2026-09-15T12:00:02Z" });
  expect(streamingTerminalGroup(snapshot, logs, flow)).toBeNull();
});

test("background repo pulls yield and prevent overlapping pulls, including after failures", async () => {
  const body = server.slice(server.indexOf("async function pullWarmedRepo()"), server.indexOf("setInterval(pullWarmedRepo,"));
  let release: (value: string) => void = () => {};
  let fail = false;
  const commands: string[] = [];
  const pull = new Function("runGitAsync", `
    let warmedRepoPulling = false;
    const serverHasActiveWork = () => true;
    const existsSync = () => true;
    const repoCheckoutDir = "/repo";
    const getSetting = () => "repo-url";
    const console = { warn() {} };
    ${body}
    return pullWarmedRepo;
  `)((args: string[]) => {
    commands.push(args.join(" "));
    if (fail) return Promise.reject(new Error("network failure"));
    if (args[0] === "remote") return new Promise<string>((resolve) => { release = resolve; });
  ...
  release("repo-url");
  await first;
  expect(commands).toEqual(["remote get-url origin", "worktree prune", "pull --ff-only"]);
  fail = true;
  await pull();
  await pull();
  expect(commands.filter((cmd) => cmd === "remote get-url origin")).toHaveLength(3);
});

test("streaming updates replace only the active row and preserve older rows", () => {
  const source = app.slice(app.indexOf("function updateStreamingTerminal("), app.indexOf("export function renderLogs("));
  const older = { label: "older row" };
  const replacement = { label: "updated row" };
  const group = { id: 2, message: "hello world" };
  const cache = new Map();
  const terminal = {
    children: [older, {}],
    _streamingSnapshot: { group: {}, log: { message: "hello world" }, message: "hello" },
    _flowLogNodeCache: cache,
    _flowLogSignature: "old",
    scrollTop: 10,
  };
  const row = { parentNode: terminal, replaceWith(node: object) { terminal.children[1] = node; } };
  terminal.children[1] = row;
  cache.set("tail", { node: row });
  const clamped: object[] = [];
  const update = new Function("state", "streamingTerminalGroup", "document", "applyTerminalMessageClamps", `
    const agentWorkingForFlow = () => true;
    const terminalTraceInteractionActive = () => false;
    const terminalGroupRenderKey = () => "tail";
    const terminalAtLatest = () => false;
    const appendTerminalBlock = () => {};
    const terminalGroupNodeSignature = () => "updated";
    const scrollTerminalToLatestNow = () => { throw new Error("must preserve scroll"); };
    ${source}
    return updateStreamingTerminal;
  `)(state, () => group, { createDocumentFragment: () => ({ lastElementChild: replacement }) }, (node: object) => clamped.push(node));
  expect(update(terminal, [], { id: "streaming-dom-test" }, {})).toBe(true);
  expect(terminal.children).toEqual([older, replacement]);
  expect(terminal.children[0]).toBe(older);
  expect(clamped).toEqual([replacement]);
  expect(terminal.scrollTop).toBe(10);
  expect(update(terminal, [], {}, { force: true })).toBe(false);
});

test("selects Prism grammars for file previews", () => {
  expect(filePreviewLanguage("apps/express/mcp/observability.ts")).toBe("typescript");
  expect(filePreviewLanguage("component.tsx")).toBe("tsx");
  expect(filePreviewLanguage("config.yaml")).toBe("yaml");
  expect(filePreviewLanguage("README.txt")).toBe("");
});

test("fades file previews and waits for resolved metadata", () => {
  expect(css).toContain("transition: opacity 200ms ease;");
  expect(app).toContain('title.textContent = "";');
  expect(app).toContain('path.textContent = "";');
  expect(app).toContain("}, 200);");
});

describe("Turbopump pane markup", () => {
  test.each(["agent:tool", "agent:tool-result", "agent:output", "shell:command", "shell:output", "serve"])(
    "keeps JSON in %s as ordinary preformatted output", (source) => {
      class OutputNode {
        ...
        children: OutputNode[] = [];
        classList = { add: () => {} };
        replaceChildren(...children: OutputNode[]) { this.children = children; }
        appendChild(child: OutputNode) { this.children.push(child); }
        setAttribute() {}
        addEventListener() {}
      }
      ...
      Object.defineProperty(globalThis, "document", {
        configurable: true,
        value: {
          createElement: (tag: string) => { const node = new OutputNode(tag); nodes.push(node); return node; },
          createTextNode: (text: string) => Object.assign(new OutputNode("#text"), { textContent: text }),
        },
      });
      try {
        const message = '{\n  "a": {"value":1},\n  "b": [2]\n}';
        appendTerminalBlock(new OutputNode("fragment"), {
          source, id: 1, flowId: "json-tool-test", createdAt: "2026-09-09T12:00:00Z", message,
        });
        const body = nodes.find((node) => node.className === "terminal-entry-body")!;
        expect(body.tagName).toBe("pre");
        expect(body.innerHTML).not.toContain("<details");
        expect(body.innerHTML || body.children.map((child) => child.textContent).join("")).toBe(message);
      } finally {
  ...
  test("colors reconciled Codex turn notices as errors while retaining their status source", () => {
    for (const message of [
      "turn stopped: reconciled Codex thread status idle",
      "turn stopped: reconciled Codex thread status notLoaded",
      "turn failed: reconciled Codex thread status systemError",
    ]) {
      expect(logMeta("agent:status", "", message)).toMatchObject({ tone: "error", label: "status" });
    }
    expect(logMeta("agent:status", "", "turn completed").tone).toBe("status");
    expect(logMeta("user", "", "turn stopped: reconciled Codex thread status idle").tone).toBe("user");
  });

  test("does not treat a compaction turn end as notification-worthy", () => {
    const turnEnd = { id: 3, source: "agent:status", message: "turn completed" };
    expect(isCompactionTurnEndedLog(turnEnd, [
      { id: 1, source: "user", message: "/compact" },
      { id: 2, source: "agent:status", message: "context compacted" },
      turnEnd,
    ])).toBe(true);
    expect(isCompactionTurnEndedLog(turnEnd, [
      { id: 1, source: "user", message: "finish the task" },
      { id: 2, source: "agent:message", message: "Done." },
      turnEnd,
    ])).toBe(false);
  });

  test.each(["user", "user:queued"])("applies the flow Markdown toggle to %s messages", (source) => {
    class MessageNode {
      className = "";
      innerHTML = "";
      children: MessageNode[] = [];
      classList = { add: () => {} };
      replaceChildren(...children: MessageNode[]) { this.children = children; }
      appendChild(child: MessageNode) { this.children.push(child); }
      querySelectorAll() { return []; }
    }
    const descriptor = Object.getOwnPropertyDescriptor(globalThis, "document");
    const previousRaw = state.rawAgentMarkdown;
    Object.defineProperty(globalThis, "document", {
      configurable: true,
      value: { createElement: () => new MessageNode() },
    });
    state.rawAgentMarkdown = true;
    try {
      const fragment = new MessageNode();
      appendTerminalBlock(fragment, {
        source, flowId: "json-user-test", createdAt: "2026-09-09T12:00:00Z",
        message: 'Please check this:\n{"a":{"value":1},"b":[2]}\nThanks',
      });
      const body = fragment.children[0]!.children[1]!;
      expect(body.innerHTML).toContain('Please check this:');
      expect(body.innerHTML).toBe('<pre class="terminal-raw-markdown">Please check this:\n{"a":{"value":1},"b":[2]}\nThanks</pre>');

      state.rawAgentMarkdownByFlow.set("json-user-test", false);
      const rendered = new MessageNode();
      appendTerminalBlock(rendered, {
        source, flowId: "json-user-test", createdAt: "2026-09-09T12:00:00Z",
        message: '**Please check this:**\n{"a":{"value":1},"b":[2]}\nThanks',
      });
      const renderedBody = rendered.children[0]!.children[1]!;
      expect(renderedBody.innerHTML).toContain('<strong>Please check this:</strong>');
      expect(renderedBody.innerHTML).toContain('markdown-json-block');
      expect(renderedBody.innerHTML).not.toContain('terminal-raw-markdown');
      expect(renderedBody.innerHTML).toEndWith('Thanks');
    } finally {
      state.rawAgentMarkdown = previousRaw;
      state.rawAgentMarkdownByFlow.delete("json-user-test");
      if (descriptor) Object.defineProperty(globalThis, "document", descriptor);
    }
  });

  test("copies original JSON regardless of collapsed fields and keeps raw output available", async () => {
    const source = ' {"a":{"id":9007199254740993},"b":[1,2]}\n';
    ...
    try {
      await copyMarkdownCodeBlock({ closest: () => ({ dataset: { codeSource: source } }) });
      await copyMarkdownCodeBlock({ closest: () => ({ dataset: {}, querySelector: () => ({ textContent: "const a = 1;" }) }) });
      expect(copied).toEqual([source, "const a = 1;"]);
      expect(renderTerminalMarkdownOutput(source)).toContain('<summary><span class="markdown-json-key">a</span>: <span class="markdown-json-bracket-object">{</span><span class="markdown-json-collapsed-preview"> <span class="markdown-json-key">id</span> <span class="markdown-json-bracket-object">}</span>,</span></summary>');
      expect(renderTerminalMarkdownOutput(source, { raw: true })).toBe(`<pre class="terminal-raw-markdown">${source}</pre>`);
      expect(renderTerminalStreamingTextOutput('{"a":')).not.toContain('<details');
    } finally {
  ...
  test("navigates session images without wrapping, deduplicates, and resets zoom only when changing images", () => {
    ...
    const buttons = [
      { disabled: false, dataset: { imagePreviewStep: "-1" } },
      { disabled: false, dataset: { imagePreviewStep: "1" } },
    ];
    ...
    try {
      ...
      const zoomAtEnd = properties.get("--image-preview-scale");
      let prevented = false;
      ...
      expect(count.textContent).toBe("2 / 2");
      expect(properties.get("--image-preview-scale")).toBe(zoomAtEnd);
      ...
      imagePreview.handleImagePreviewKeydown({ key: "ArrowRight", preventDefault() {} });
      expect(image.src).toBe("/b.png");
      imagePreview.handleImagePreviewWheel({ deltaY: -1, clientX: 30, clientY: 20, preventDefault() {} });
  ...
  test("adds an agent output toolbar that can hide trace messages", () => {
    ...
    expect(html).toContain('aria-label="Show raw Markdown"');
    ...
    expect(app).toContain("agentTraceHiddenByFlow: new Map()");
    expect(app).toContain("rawAgentMarkdownByFlow: new Map()");
    expect(app).toContain('state.agentTraceHiddenByFlow.set(flowId, !agentTraceHiddenForFlow(flowId))');
    expect(app).toContain('state.rawAgentMarkdownByFlow.set(flowId, !rawAgentMarkdownForFlow(flowId))');
    expect(app).toContain("renderLogs(flowId, { force: true, preserveScrollTop: true });");
    expect(app).toContain('toggleAgentOutput(button.closest(".agent-output-toolbar")?.dataset.flowId, button.dataset.agentOutput);');
    expect(app).toContain("function renderableTerminalGroups(groups, options = {})");
    expect(app).toContain("function agentOutputSimpleMode(flowId)");
    ...
    const toolbarCss = css.slice(css.indexOf(".agent-output-toolbar {"), css.indexOf(".agent-output-button {"));
    expect(toolbarCss).toContain("background: var(--panel);");
    expect(toolbarCss).toContain(".agent-output-toolbar::after");
    expect(toolbarCss).toContain("bottom: -1px;");
  ...
  test("previews the latest tool call instead of its results and counts calls since the previous thought", () => {
    ...
    try {
      ...
      expect(groups.map((group) => group.id)).toEqual([1, 2, 5]);
  ...
  test("does not preview tool results when the current prompt has no tool calls", () => {
    const previous = state.agentTraceHidden;
    state.agentTraceHidden = true;
    try {
      const groups = [
        { id: 1, source: "agent:tool", message: "previous call" },
        { id: 2, source: "user", message: "continue" },
        { id: 3, source: "agent:tool-result", message: "result" },
        { id: 4, source: "agent:output", message: "output" },
      ];
      expect(renderableTerminalGroups(groups, { agentWorking: true }).map((group: { id: number }) => group.id)).toEqual([2]);
      state.agentTraceHidden = false;
      expect(renderableTerminalGroups(groups, { agentWorking: true })).toEqual(groups);
    } finally {
      state.agentTraceHidden = previous;
    }
  });
  ...
  test("filters environment names without removing saved values and clears search with Escape", () => {
    const previous = {
      settingsContent: els.settingsContent,
    ...
    const otherSections = [{ inert: false }, { inert: false }];
    const inside = {};
    const outside = {};
    ...
    Object.assign(els, {
      settingsContent: {
        classList: { toggle: (_name: string, active: boolean) => { searchMode = active; } },
        querySelectorAll: () => otherSections,
      },
    ...
    try {
      ...
      expect(otherSections.every((section) => section.inert)).toBe(true);
  ...
  test("updates environment results immediately while typing and clearing search", () => {
    ...
    const row = {
      ...
      getBoundingClientRect: () => { throw new Error("Filtering must not force layout"); },
      animate: () => { throw new Error("Filtering must not wait for animations"); },
    };
  ...
  test("exposes agent developer instructions in Settings", () => {
    ...
    expect(server).toContain('Example body: {"prUrl":"https://github.com/org/repo/pull/123","linearIssueId":"ENG-123"}');
    expect(server).toContain("Each field in the body is optional.");
    expect(server).toContain("function normalizeLinearIssueId(value: unknown)");
  ...
  test("shows worktree cleanup cards in Settings and deletes trace data with the workspace", () => {
    ...
    expect(app).toContain('const data = await api("/api/checkouts");\n    if (data.flows) setFlows(data.flows);');
    ...
    expect(app).toContain("function nextSessionTicket(flow)");
    expect(app).toContain('const nextTicket = flow?.id === state.selectedFlowId ? nextSessionTicket(flow) : null;');
    expect(app).toContain('return tickets[index - 1] || tickets[index + 1] || null;');
    expect(app).toContain("if (currentNextTicket) await openTicketInFlowPane(currentNextTicket);");
    ...
    expect(app).toContain('await api(`/api/checkouts/${encodeURIComponent(name)}`, {');
    expect(app).toContain('signal: AbortSignal.timeout(15000)');
    ...
    expect(server).toContain("function deleteMissingWorktreeSessions()");
    expect(server).toContain("if (flow.parentFlowId || !flow.checkoutPath || existsSync(flow.checkoutPath)) continue;");
    expect(server).toContain("deleteMissingWorktreeSessions();\n  const clearAfterLogIds = shellOutputClearAfterLogIds();");
    expect(server).toContain('throw new Error("Session was deleted because its worktree no longer exists.");');
    expect(server).toContain('return json({ flows: listClientFlows(), checkouts: listWorktrees() });');
    ...
    expect(server).toContain("await worktreeCleanup.stage(target);");
    expect(server).toContain("void worktreeCleanup.run();");
    expect(server).toContain('runGitAsync(["worktree", "prune", "--expire", "now"], repoCheckoutDir, process.env, 5000)');
  ...
  test("backs off noisy Linear connectivity failures", () => {
    ...
    expect(server).toContain("const allLinearIssuesStmt = db.query(\"select identifier, issueJson, assigned from linear_issues order by updatedAt desc\");");
    expect(server).toContain("upsertLinearIssueStmt.run(identifier, JSON.stringify(issue), issue.updatedAt || now());");
    expect(server).toContain("function linearTicketsFromDb()");
    expect(server).toContain("function cachedLinearTicketsPayload()");
    expect(server).toContain('if (url.searchParams.get("cached") === "1") return json(cachedLinearTicketsPayload());');
    expect(server).toContain("const payload = cachedLinearTicketsPayload();");
    expect(server).toContain("if (payload.issues.length) {");
    expect(server).toContain("return payload;");
  ...
  test("paginates assigned Linear tickets", () => {
    ...
    expect(server).toContain("return linearTicketsPayload(viewer, false, workflowStates);");
  });

  test("keeps Linear tickets without Turbopump sessions visible", () => {
    expect(server).toContain("const flow = flowsByIssue.get(issue.identifier);");
    expect(server).toContain("flowId: flow?.id ?? \"\"");
    expect(server).not.toContain("function isDoneLinearIssue(issue: LinearIssue)");
    expect(server).not.toContain("function isDuplicateLinearIssue(issue: LinearIssue)");
  ...
  test("refreshes cached Linear descriptions and comments from the ticket refresh button", () => {
    ...
    expect(server).toContain("if (!issue?.identifier || isDeletedLinearIssue(issue)) continue;");
    expect(server).toContain("function isLinearIssueNotFoundError(error: unknown)");
    expect(server).toContain("function invalidateLinearIssue(identifier: string)");
    expect(server).toContain("deleteLinearIssueStmt.run(issueId);");
    expect(server).toContain('updateFlow(flow.id, { linearIssueId: "", linearIssueUrl: "", linearStatus: "" });');
    expect(server).toContain("if (isLinearIssueNotFoundError(error)) {\n      invalidateLinearIssue(identifier);\n      return null;\n    }");
    expect(server).toContain("function cachedIssueLooksValid(issue: LinearIssue)");
  ...
  test("creates sessions first and lets them create a Linear ticket later", () => {
    ...
    expect(app).toContain('title: flow.title || "new session"');
    expect(app).toContain('localSession: true');
    expect(app).toContain("ticket.localSession && !state.pinnedLinearIssues.has(ticket.identifier)");
    expect(app).toContain("ticket.identifier === identifier && ticket.localSession");
    expect(app).toContain('data-create-linear-ticket="true"');
    expect(app).toContain('aria-label="${createLinearLabel}" title="${createLinearLabel}"');
    ...
    expect(app).toContain("const pinButton = renderLinearPinButton(issue);");
    expect(app).toContain('<div class="linear-issue-kicker">');
    expect(css).toContain(".linear-issue-kicker {\n  display: flex;\n  align-items: center;\n  gap: 8px;\n  height: 18px;");
    expect(css).toContain(".linear-issue-kicker > .linear-pin-toggle:only-child {\n  margin-left: -4px;\n}");
    expect(css).toContain(".linear-meta {\n  display: flex;\n  align-items: center;\n  flex-wrap: wrap;\n  gap: 6px;\n  min-height: 22px;");
    expect(app).toContain('${hasLinearIssue ? `<a href="${escapeAttribute(issue.url || context.issueUrl)}" target="_blank" rel="noreferrer">${escapeHtml(issue.identifier || context.issueId)}</a>` : ""}');
    expect(app).toContain('${pinButton}\n          </div>\n          ${titleHtml}');
    expect(app).toContain('<h3>${escapeHtml(title)}</h3>\n        ${linearButton}');
    expect(app).toContain('const linearButton = hasLinearIssue\n    ? ""');
  ...
  test("avoids layout animations when opening and closing Settings", () => {
    const settingsRules = [...css.matchAll(/([^{}]+)\{([^{}]*)\}/g)]
      .filter((match) => /\.(?:settings[\w-]*|sidebar)\b/.test(match[1]!))
      .filter((match) => !match[1]!.includes(".settings-search-section"))
      .map((match) => match[2]).join("\n");
    expect(settingsRules).not.toContain("transition: grid-template-columns");
    expect(settingsRules).not.toContain("grid-template-rows 180ms ease");
    expect(settingsRules).not.toContain("padding 180ms ease");
  ...
  test("keeps input panes editable and blocks submission at submit time", () => {
    ...
    expect(app).toContain("clearPromptDraftForIssue(flowSelectionId(previousFlow), message);");
    ...
    expect(app).toContain('querySelector(".terminal-panel > .message-form > .queued-prompt-hint")');
    ...
    expect(css).toContain(
      ".agent-panel.disabled .agent-column,\n.agent-panel.disabled .shell-output-resizer,\n.agent-panel.disabled .shell-column {\n  display: none;",
    );
  ...
  test("uses git worktrees instead of copied checkouts for flow workspaces", () => {
    ...
    expect(server).toContain('await runGitAsync(["pull", "--ff-only"], repoCheckoutDir, process.env, 120000);');
    ...
    expect(server).toContain("id, linearIssueId, linearIssueUrl, title, linearStatus, agentReasoningEffort, agentServiceTier,");
    ...
    expect(server).toContain('const { target, branch, baseSha } = createWorktree(id, parsed.identifier || `session-${id.slice(0, 8)}`);');
  ...
  test("edits pane titles inline and saves them to Linear or the local session", () => {
    ...
    expect(app).toContain('`/api/linear/issues/${encodeURIComponent(issueId)}/title`');
    expect(app).toContain('`/api/flows/${encodeURIComponent(flow.id)}/meta`');
    expect(app).toContain('ticket?.localSession ? { title: nextTitle }');
    ...
    expect(server).toContain('if ("title" in body)');
    ...
    expect(css).toContain("clip-path: inset(0 round 2px);");
  });
  ...
  test("deletes a ticket from the right-click menu and Linear", () => {
    expect(app).toContain('<button class="danger" type="button" role="menuitem">Delete</button>');
    expect(app).toContain("deleteLinearTicket(ticket)");
    expect(app).toContain("window.confirm(`Delete ${identifier} from Linear?`)");
    expect(app).toContain('method: "DELETE"');
    expect(server).toContain("issueDelete(id: $id)");
    expect(server).toContain("invalidateLinearIssue(identifier);");
    expect(server).toContain('request.method === "DELETE"');
  ...
  test("lets pinned cards be manually sorted, including sessions without Linear tickets", () => {
    ...
    expect(app).toContain("const pinnedIssueIds = orderedPinnedTickets(state.linearTickets)");
  ...
  test("renders Markdown headings, inline formatting, and code blocks in Linear and Agent panes", () => {
    ...
    expect(app).toContain('["user", "user:queued", "agent", "agent:message"].includes(source)');
    ...
    expect(usesRawAgentMarkdown("user")).toBe(true);
    expect(usesRawAgentMarkdown("user:queued")).toBe(true);
    ...
    expect(app).toContain('block?.dataset.codeSource ?? block?.querySelector("code")?.textContent ?? ""');
  ...
  test("groups Linear tickets behind collapsible status separators", () => {
    ...
    expect(app).toContain("updateTicketSelectionCards(previousIssueId, state.selectedLinearIssueId);");
  ...
  test("lets ticket cards move between Linear status groups with drag and drop", () => {
    ...
    expect(app).toContain("ticket && !ticket.localSession && group.stateId");
    ...
    expect(server).toContain("workflowStates(first: 250) @include(if: $includeWorkflowStates)");
    expect(server).toContain("return linearTicketsPayload(viewer, false, workflowStates);");
    expect(server).toContain("cacheLinearIssue(issue, { assigned: true });");
    expect(server).toContain("if (!fetchedIdentifiers.has(row.identifier)) setLinearIssueAssignedStmt.run(0, row.identifier);");
    expect(server).toContain("if (!cachedIssueLooksValid(issue)) {");
    expect(server).toContain("invalidateLinearIssue(issue.identifier);");
    expect(server).toContain("turbopumpFlowOnly: true");
  ...
  test("keeps ticket flow status synced when flows update", () => {
    ...
    expect(app).toContain("const selected = state.flows.find((flow) => flow.id === state.selectedFlowId);");
    expect(app).toContain('localStorage.setItem("flow.selectedLinearIssueId", selectedId);');
    expect(app).toContain("const flowsByIssue = new Map");
    expect(app).toContain("const baseTickets = state.linearTickets.filter((ticket) => !ticket.localSession && (!ticket.turbopumpFlowOnly || flowCanRenderTicket(flowsByIssue.get(ticket.identifier))));");
    expect(app).toContain("const ticketIds = new Set(baseTickets.map((ticket) => ticket.identifier));");
    expect(app).toContain("function flowCanRenderTicket(flow)");
    expect(app).toContain('flow.title !== "Untitled"');
    expect(app).toContain("const canAddFlowOnlyTickets = state.linearTicketsLoaded || state.linearTickets.length > 0;");
    expect(app).toContain("if (canAddFlowOnlyTickets) {");
    expect(app).toContain("return { ...ticket, flowId };");
    expect(app).toContain("identifier: flow.linearIssueId,");
    expect(app).toContain("turbopumpFlowOnly: true,");
    expect(app).toContain("localSession: true,");
  ...
  test("shows context availability, model, reasoning effort, fast mode, and branch values on the right side of the Agent input", () => {
    ...
    expect(html).toContain('class="agent-context-model" role="button" tabindex="0" aria-haspopup="listbox" aria-label="Change agent model"');
    ...
    expect(server).toContain("linearStatus, agentReasoningEffort, agentServiceTier");
    expect(server).toContain('linearIssue?.state?.name || "",\n      defaultAgentProviderKind() === "codex" ? codexDefaultReasoningEffort : "low",');
    ...
    expect(app).toContain('model.setAttribute("aria-disabled", String(!modelInteractive || modelDisabled));');
    expect(app).toContain("model.tabIndex = modelInteractive ? 0 : -1;");
    expect(app).toContain("model.onclick = modelInteractive ? () => (modelDisabled ? flashBlockedInput(input) : openModelMenu()) : null;");
    expect(app).toContain("model.onkeydown = modelInteractive ? (event) => handleModelMenuKeydown(event, modelDisabled, input) : null;");
    expect(app).toContain("function openModelMenu()");
    expect(app).toContain("if (state.slashMenuCommands) return hideSlashMenu();");
    expect(app).toContain("function handleModelMenuKeydown(event, blocked = false, input = promptInput())");
    ...
    expect(app).not.toContain('input.value = "/model";');
    const modelCss = css.slice(css.indexOf(".agent-context-model,"), css.indexOf(".agent-context .agent-provider-icon"));
    expect(modelCss).not.toContain("display: inline;");
    expect(modelCss).not.toContain("appearance: none;");
    expect(css).toContain('.agent-context-model[role="button"] {\n  cursor: pointer;\n}');
  ...
  test("lets agents update flow metadata including PR URL", () => {
    ...
    expect(server).toContain("const issue = await fetchLinearIssue(parsed.identifier);");
    expect(server).toContain("if (!issue) throw new Error(`Linear issue ${parsed.identifier} not found.`);");
    expect(server).toContain('fields.linearIssueId = parsed.identifier;');
  ...
  test("polls selected flow GitHub CI on the server and displays persisted status", () => {
    ...
    expect(server).toContain("setInterval(() => void pollSelectedGithubCiStatus(), 2000);");
  ...
  test("interrupts active agent turns instead of killing the app server", () => {
    ...
    expect(app).toContain("function canSubmitShellCommand()");
    expect(app).toContain("function flashBlockedInput(input)");
    expect(app).toContain('async function uploadAgentImages(files, flowId = "")');
    ...
    expect(app).toContain('event.target?.closest?.(".terminal-split-pane:not([hidden])")?.querySelector(".message-input-split")');
    expect(app).toContain("function setAgentImageDragActive(active, input = promptInput())");
    ...
    expect(app).toContain("const slashCommand = message.startsWith(\"/\");");
    expect(app).toContain("const queuedImages = slashCommand ? [] : [...state.pendingAgentImages];");
    expect(app).toContain("const queuedMessage = slashCommand\n    ? message\n    : agentMessageWithImages");
    expect(app).toContain("message: queuedMessage,");
    expect(app).toContain("const submittedImages = slashCommand ? [] : [...state.pendingAgentImages];");
    expect(app).toContain('const agentMessage = slashCommand ? message : agentMessageWithImages(message || "Use the attached image context.", submittedImages);');
    expect(app).toContain("if (!slashCommand) state.pendingAgentImages = [];");
    expect(app).toContain("if (!slashCommand) state.pendingAgentImages = [...queuedImages, ...state.pendingAgentImages];");
    expect(app).toContain("if (!slashCommand) state.pendingAgentImages = [...submittedImages, ...state.pendingAgentImages];");
    ...
    expect(app).toContain("setAgentImageDragActive(true, input);");
    expect(html).toContain('class="agent-image-context agent-image-context-split" aria-label="Attached image context" hidden');
    expect(app).toContain('const flowId = event.target?.closest?.(".terminal-split-pane:not([hidden])") ? splitAgentPaneFlowId() : "";');
    expect(app).toContain("const images = flowId ? state.pendingSplitAgentImages : state.pendingAgentImages;");
  ...
  test("lets the Agent and Linear panes be resized with the separator", () => {
    ...
    expect(css).not.toContain("padding: 14px 14px 42px;");
    expect(css).toContain("background: var(--panel);\n  z-index: 1;");
  ...
  test("renders command execution logs as a single shell prompt line", () => {
    ...
    expect(app).toContain("body.innerHTML = renderInlineMarkdown(message, {");
  ...
  test("loads older terminal rows from the load more button", () => {
    expect(app).toContain("await Promise.all([ticketsLoading, connectWs()]);");
    ...
    expect(app).toContain("const AGENT_TRACE_INITIAL_TURN_COUNT = 12;");
    expect(app).toContain("const AGENT_TRACE_TURN_PAGE_SIZE = 12;");
    ...
    expect(app).toContain("if (pagesLoaded >= 3 || terminalRowCount(groups) >= AGENT_TRACE_INITIAL_TURN_COUNT) break;");
  ...
  test("fades out before swapping sessions and cancels stale switches", async () => {
    // @ts-expect-error Browser JavaScript modules do not provide declarations.
    const { animateTicketSwitch } = await import("../public/js/tickets.js");
    const previousWindow = Object.getOwnPropertyDescriptor(globalThis, "window")!;
    const previousPane = els.flowPane;
    const previousState = { ticketSwitchFadeTimer: state.ticketSwitchFadeTimer, ticketSwitchFadingOut: state.ticketSwitchFadingOut };
    ...
    Object.assign(state, { ticketSwitchFadeTimer: 0, ticketSwitchFadingOut: false });
    const finishTimer = (delay: number) => {
    ...
    try {
      animateTicketSwitch(() => rendered.push("first"));
      expect(rendered).toEqual([]);
      expect(state.ticketSwitchFadingOut).toBe(true);
      expect(classes.has("ticket-switch-fade-out")).toBe(true);
      animateTicketSwitch(() => rendered.push("latest"));
      expect(timers.size).toBe(1);
      finishTimer(50);
      expect(rendered).toEqual(["latest"]);
      expect(state.ticketSwitchFadingOut).toBe(false);
      expect(classes.has("ticket-switch-fade-out")).toBe(false);
      expect(classes.has("ticket-switch-fade-in")).toBe(true);
      finishTimer(100);
      expect(classes.size).toBe(0);
      expect(state.ticketSwitchFadeTimer).toBe(0);

      animateTicketSwitch(() => rendered.push("cancelled"));
      animateTicketSwitch(() => rendered.push("same session"), false);
      expect(rendered).toEqual(["latest", "same session"]);
      expect(timers.size).toBe(0);
      expect(state.ticketSwitchFadingOut).toBe(false);
      reducedMotion = true;
      animateTicketSwitch(() => rendered.push("reduced motion"));
      expect(rendered.at(-1)).toBe("reduced motion");
      expect(timers.size).toBe(0);
      expect(classes.size).toBe(0);
    } finally {
      Object.defineProperty(globalThis, "window", previousWindow);
      els.flowPane = previousPane;
      Object.assign(state, previousState);
    }
  });
  ...
  test("keeps ticket switching scoped to the selected panes", () => {
    ...
    expect(app).toContain("function animateTicketSwitch(renderContent, changed = true)");
    expect(app).toContain("updateTicketSelectionCards(previousIssueId, state.selectedLinearIssueId);");
    expect(app).toContain("updateTicketSelectionCards(previousIssueId, ticket.identifier);");
    ...
    expect(selectFlowBody).toContain("}, previousIssueId !== state.selectedLinearIssueId);");
    expect(openTicketBody).toContain("}, previousIssueId !== ticket.identifier && !ticket.pendingSession);");
    expect(selectFlowBody).toContain("scheduleSelectedFlowPaneRender(state.selectedLinearIssueId, id);");
    ...
    expect(app).toContain('if (!options.light && flow?.linearIssueId) void loadLinearDetail(issueId);');
  ...
  test("uses turn boundaries for collapsed trace elapsed time", () => {
    const groups = terminalGroups(
      [
        { id: 1, flowId: "flow-1", source: "user", message: "keep watching the PR", createdAt: "2026-07-02T20:43:00.000Z" },
        { id: 2, flowId: "flow-1", source: "agent:status", message: "turn started abc", createdAt: "2026-07-02T20:43:00.100Z" },
        { id: 3, flowId: "flow-1", source: "agent:reasoning", message: "checking", createdAt: "2026-07-02T20:52:59.000Z" },
        { id: 4, flowId: "flow-1", source: "agent:tool", message: "bun test", createdAt: "2026-07-02T20:52:59.500Z" },
        { id: 5, flowId: "flow-1", source: "agent:message", message: "fixed", createdAt: "2026-07-02T20:53:00.000Z" },
        { id: 6, flowId: "flow-1", source: "agent:status", message: "turn completed", createdAt: "2026-07-02T20:53:00.000Z" },
      ],
      { agentStatus: "idle" },
    );
  ...
  test.each(["user", "user:queued"])("keeps streaming output above an interleaved %s message", (source) => {
    const logs = [
      { id: 1, source: "user", message: "investigate" },
      { id: 2, source: "agent:status", message: "turn started abc" },
      { id: 3, source: "agent:message", message: "I'll record that distinction " },
      { id: 4, source, message: "sure your func didnt run in prod?" },
      { id: 5, source: "agent:message", message: "in the PR." },
    ].map((log) => ({ ...log, flowId: "flow-1", createdAt: "2026-09-14T19:21:00.000Z" }));
    const groups = terminalGroups(logs, { id: "flow-1", agentStatus: "running" });
    const reply = groups.find((group: { source: string }) => group.source === "agent:message");
    expect(groups.map((group: { id: number }) => group.id)).toEqual([1, 3, 4]);
    expect(reply.message).toBe("I'll record that distinction in the PR.");
    expect(reply.logIds).toEqual([3, 5]);
    expect(reply.liveStreaming).toBe(true);
    expect(reply.turnPending).toBe(true);

    for (const boundary of [
      { source: "agent:message-boundary", message: "" },
      { source: "agent:status", message: "turn completed" },
      { source: "agent:status", message: "turn started def" },
    ]) {
      const completed = terminalGroups([
        ...logs,
        { ...logs[0], ...boundary, id: 6 },
        { ...logs[0], id: 7, source: "agent:message", message: "A separate reply." },
      ], { agentStatus: "idle" });
      expect(completed.filter((group: { source: string }) => group.source === "agent:message").map((group: { message: string }) => group.message))
        .toEqual(["I'll record that distinction in the PR.", "A separate reply."]);
    }
  });
  ...
  test("supports slash command suggestions and dispatch", () => {
    ...
    expect(app).toContain('const defaultCodexModel = "gpt-6-sol";');
    expect(app).toContain('const defaultCodexReasoningEffort = "medium";');
    expect(app).toContain('"gpt-6-sol",');
    ...
    expect(app).toContain("function resizeMessageInput(input = promptInput())");
    ...
    expect(app).toContain('if (command.startsWith("/model ")) {');
    expect(app).toContain("void submitPromptCommand(command);");
    expect(app).toContain("export async function submitPromptCommand(command)");
    expect(app).toContain("if (!message.startsWith(\"/\") || state.messageSubmitting) return false;");
    expect(app).toContain('if (message.startsWith("/model ") && flowAgentRunning(selectedFlow()))');
    expect(server).toContain("function parseSlashCommand(message: string)");
    expect(server).toContain('"thread/compact/start"');
    expect(server).toContain('codexThreadParams(flow, "clear", runtime.codexProjectId)');
    ...
    expect(server).toContain('["running", "interrupting"].includes(flow.agentStatus)');
    ...
    expect(startAgentBody).toContain('if (message.startsWith("/") && (await handleSlashCommand(flow, userMessage))) return;');
    ...
    expect(server).toContain('primary: windows.find((window) => duration(window) === 300)');
    expect(server).toContain('secondary: windows.find((window) => duration(window) === 10_080)');
    ...
    expect(server).toContain('["Credits", statusCredits(rateLimits.credits)]');
    expect(server).not.toContain('["5h left",');
  ...
  test("opens ticket search with Cmd+K and reveals collapsed tickets", () => {
    const body = app.match(/export function handleCommandK\(event\) \{([\s\S]*?)\n\}/)?.[1];
    expect(body).toBeDefined();
    const calls: unknown[] = [];
    const shortcutState = { ticketDrawerHidden: true };
    const handle = new Function("state", "setTicketDrawerHidden", "openTicketSearch", "event", body!);
    const event = {
    ...
    const run = (overrides = {}) => handle(shortcutState, (hidden: boolean) => calls.push(hidden), () => calls.push("search"), { ...event, ...overrides });
    expect(run()).toBe(true);
    expect(calls).toEqual(["prevent", "stop", false, "search"]);
    calls.length = 0;
    shortcutState.ticketDrawerHidden = false;
    expect(run()).toBe(true);
    expect(calls).toEqual(["prevent", "stop", "search"]);
    calls.length = 0;
    expect(run({ repeat: true })).toBe(true);
    expect(calls).toEqual(["prevent", "stop"]);
    calls.length = 0;
    for (const overrides of [{ metaKey: false }, { ctrlKey: true }, { altKey: true }, { shiftKey: true }, { isComposing: true }, { key: "e" }]) {
      expect(run(overrides)).toBe(false);
    }
  ...
  test("toggles the shell pane with Cmd+Backslash", () => {
    ...
    expect(app).toContain('const shellPanel = els.flowPane.querySelector(".shell-command-panel:not(.shell-command-panel-split)");');
  ...
  test("supports separate reverse search histories for prompts and shell commands", () => {
    ...
    expect(app).toContain("syncTicketInputState(selectionId);");
    expect(app).toContain("function renderHistorySearchIndicator(input = document.activeElement)");
    expect(app).toContain('input?.closest?.(".message-form")');
    expect(app).toContain("if (form && indicator.parentElement !== form) form.prepend(indicator);");
    ...
    expect(app).toContain("resizeMessageInput(input);");
    ...
    expect(app).toContain('const splitPrompt = els.flowPane.querySelector(".message-input-split");');
    expect(app).toContain('const splitShell = els.flowPane.querySelector(".shell-input-split");');
    expect(app).toContain('if (!splitPrompt?.closest(".terminal-split-pane")?.hidden && (focused === primaryPrompt || focused === splitPrompt)) {');
    expect(app).toContain('(focused === primaryPrompt ? splitPrompt : primaryPrompt)?.focus({ preventScroll: true });');
    expect(app).toContain('if (!splitShell?.closest(".shell-output-split-pane")?.hidden && (focused === primaryShell || focused === splitShell)) {');
    expect(app).toContain('(focused === primaryShell ? splitShell : primaryShell)?.focus({ preventScroll: true });');
  ...
  test("clears sent queued drafts for sessions without Linear tickets", () => {
    expect(app).toContain("const issueId = flowSelectionId(flow);");
    expect(app).toContain("const inputState = ticketInputState(flowSelectionId(flow));");
    expect(app).toContain("clearPromptDraftForIssue(flowSelectionId(previousFlow), message);");
  });
  ...
  test("toggles a matching split session with Cmd+Slash", () => {
    expect(html).toContain("<kbd>Cmd + /</kbd>");
    expect(html).toContain("<dd>Toggle split session</dd>");
    expect(app).toContain("function handleSplitPaneShortcut(event)");
    expect(app).toContain('event.shiftKey || event.key !== "/"');
    expect(app).toContain("if (flow && isSplitPaneOpen(flow.id, kind)) closeSplitPane(kind);");
    expect(app).toContain("else void openSplitPane(kind);");
    expect(app).toContain('document.addEventListener("keydown", handleSplitPaneShortcut, true);');
    expect(app).not.toContain("handleCloseSplitPaneShortcut");
    expect(html).not.toContain("<kbd>Ctrl + D</kbd>");
  });
  ...
  test("labels Codex threads with the repository project name", () => {
    expect(server).toContain('const name = getSetting("repoName") || repoBasename(getSetting("repoUrl"));');
    expect(server).toContain('sendCodexRequest(runtime, "project/create"');
    expect(server).toContain('roots: [{ path: resolve(repoCheckoutDir) }]');
    expect(server).toContain('...(projectId ? { projectId } : {})');
    expect(server).toContain('sendCodexRequest(runtime, "thread/metadata/update", { threadId: thread.id, projectId: runtime.codexProjectId })');
  });
  ...
  test("opens a second agent or shell pane from the input prefixes", () => {
    ...
    expect(css).toContain(".terminal-panel.agent-split-open .agent-column {\n  grid-template-rows: minmax(0, var(--agent-split-size, 50%)) minmax(0, 1fr);");
    expect(css).toContain(".terminal-panel.shell-split-open .shell-column {\n  grid-template-rows: minmax(0, 1fr) minmax(0, 1fr);");
    expect(css).toContain(
    ...
    expect(html).toContain('class="message-form-split-resizer" role="separator"');
    expect(html).toContain('<div class="queued-prompt-hint" aria-live="polite" hidden>message queued</div>');
    ...
    expect(app).toContain("renderAgentContext(agentOpen ? companion : null, terminalPane, input, { modelMenu: false });");
    expect(app).toContain('${flowAgentRunning(companion) ? "queued-prompt" : "message"}');
    expect(app).toContain('if (queued && !prefixGlyph?.querySelector(".queued-prompt-spinner"))');
    expect(app).toContain('splitPromptInput().addEventListener("beforeinput", handleSplitQueuedPromptBeforeInput);');
    expect(app).toContain('if (event.key === "Escape" || event.key === "Backspace") clearSplitQueuedPrompt();');
    expect(app).toContain("input.dataset.queuedPrompt = queuedPrompt.message;");
    expect(app).toContain('els.flowPane.querySelector(".terminal-panel > .message-form").addEventListener("submit"');
    expect(app).toContain('return active?.matches?.(".message-input") ? active : promptInput();');
    expect(app).toContain('if (form && menu.parentElement !== form) form.append(menu);');
    expect(app).toContain('splitPromptInput().addEventListener("input", () => {');
    expect(app).toContain('splitPromptInput().addEventListener("keydown", (event) => {\n  const input = event.currentTarget;\n  if (handleSplitQueuedPromptKeydown(event)) return;\n  if (handleHistorySearchKeydown(event)) return;');
    expect(app).toContain("if (handleInputHistoryNavigationKeydown(event)) return;");
    expect(app).toContain('if (input.matches(".message-input-split")) {');
    expect(app).toContain('(!input.matches(".message-input-split") && promptQueuedForSelectedFlow())');
    expect(css).toContain('    "images"\n    "input"\n    "context";\n  border-bottom: 1px solid var(--line);');
    expect(css).toContain(".prompt-input-pane.prompt-input-pane-split {\n  grid-area: input;");
    expect(css).toContain(".message-form-split .agent-context-diff,\n.message-form-split .agent-context-branch {\n  display: none;");
    expect(css).toContain(".message-form-split-resizer {\n  position: absolute;");
    expect(css).toContain(".message-form-split .message-input-split {\n  cursor: text;");
    expect(app).toContain('querySelector(".message-form-split").addEventListener("pointerdown", startAgentSplitResize);');
    expect(app).toContain('if (event.target.closest("textarea, button, a, .slash-menu, .agent-image-context")) return;');
    expect(app).toContain('const anchoredTerminals = [...column.querySelectorAll(".terminal")].filter(terminalAtLatest);');
    expect(app).toContain("anchoredTerminals.forEach(scrollTerminalToLatestNow);");
    expect(app).toContain('column?.style.setProperty("--agent-split-size", `${applied}%`);');
    expect(app).toContain("flowSelectionId(flow) === identifier && !flow.parentFlowId");
  ...
  test("resizes the upper agent pane to the lower composer and back", async () => {
    // @ts-expect-error Browser JavaScript modules do not provide declarations.
    const { startAgentSplitResize, handleAgentSplitResizeKeydown } = await import("../public/js/split.js");
    const previousPane = els.flowPane;
    const previousWindow = Object.getOwnPropertyDescriptor(globalThis, "window");
    const previousDocument = Object.getOwnPropertyDescriptor(globalThis, "document");
    const listeners = new Map<string, (event: { clientY: number }) => void>();
    let size = "50%";
    let ariaValue = "50";
    let scrollTop = 500;
    const lowerTerminal = {
      scrollHeight: 1000,
      get clientHeight() { return 1000 - Number.parseFloat(size) * 10; },
      get scrollTop() { return scrollTop; },
      set scrollTop(value: number) { scrollTop = Math.min(value, this.scrollHeight - this.clientHeight); },
    };
    const column = {
      style: { setProperty: (_name: string, value: string) => { size = value; }, getPropertyValue: () => size },
      getBoundingClientRect: () => ({ height: 1000 }),
      querySelector: () => ({ getBoundingClientRect: () => ({ height: 500 }) }),
      querySelectorAll: () => [lowerTerminal],
    };
    els.flowPane = { querySelector: (selector: string) => selector === ".agent-column" ? column : {
      setAttribute: (_name: string, value: string) => { ariaValue = value; },
    } };
    Object.defineProperty(globalThis, "window", { configurable: true, value: {
      addEventListener: (name: string, callback: (event: { clientY: number }) => void) => listeners.set(name, callback),
      removeEventListener: (name: string) => listeners.delete(name),
    } });
    Object.defineProperty(globalThis, "document", { configurable: true, value: {
      body: { classList: { add: () => {}, remove: () => {} } },
    } });
    const event = { preventDefault: () => {}, currentTarget: { closest: () => column }, target: { closest: () => null } };
    try {
      startAgentSplitResize({ ...event, clientY: 500 });
      listeners.get("pointermove")!({ clientY: 1200 });
      expect([size, ariaValue]).toEqual(["100%", "100"]);
      expect(lowerTerminal.scrollTop + lowerTerminal.clientHeight).toBe(1000);
      listeners.get("pointermove")!({ clientY: 990 });
      expect(lowerTerminal.clientHeight).toBe(10);
      expect(lowerTerminal.scrollTop + lowerTerminal.clientHeight).toBe(1000);
      listeners.get("pointermove")!({ clientY: 800 });
      expect(size).toBe("80%");
      expect(lowerTerminal.scrollTop + lowerTerminal.clientHeight).toBe(1000);
      listeners.get("pointerup")!({ clientY: 800 });
      expect(listeners.size).toBe(0);
      handleAgentSplitResizeKeydown({ ...event, key: "End" });
      expect(size).toBe("100%");
      handleAgentSplitResizeKeydown({ ...event, key: "ArrowUp" });
      expect(size).toBe("95%");
    } finally {
      els.flowPane = previousPane;
